...
namespace IdentityTest2.Controllers
{
    public class News1Controller : Controller
    {
        ...
        public ActionResult SearchForNews(string searchString, int? page)
        {
            ...
            return View(searchedNews.ToList().ToPagedList(page ?? 1, 5));
     


            //var newsList = searchedNews.ToList();  //test code to make sure the SQL statement works

            // Turn user input to a list of keywords.
            //string[] keywords = searchString.Trim().Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
            //this.keywords = keywords.ToList();
            //return View(searchedNews.ToList());
            //return View(searchedNews.ToPagedList(page ?? 1, 5));
